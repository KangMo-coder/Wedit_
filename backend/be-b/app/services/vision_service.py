    ...
    def judge(
        ...
        try:
            response = self.client.responses.parse(
                model=self.model,
                instructions=(
                    "당신은 WEDIT의 미션 인증 영상 판정 AI입니다. 사용자가 촬영한 5초 영상에서 "
                    "추출된 연속 프레임들을 보고 미션 수행 여부를 판정하세요. "
                    "사용자의 얼굴, 얼굴 일부 또는 신원이 화면에 보이는지는 인증 조건이 아닙니다. "
                    "얼굴이 전혀 나오지 않거나 가려져 있어도 손, 신체 동작, 도구나 제품 등으로 "
                    "미션의 핵심 행동이 확인되면 PASS로 판정하세요. 얼굴 미노출만을 이유로 FAIL이나 "
                    "HOLD를 반환하거나 얼굴이 보이도록 재촬영을 요구하지 마세요. "
